...
public class concreteStrategyAlumno implements IStrategy {

    @Override
    public void mostrarHorarioClases(int idAlumno, JTable tablaHorario) {
        ...
        String sql = "SELECT hc.id, c.codigo, c.nombre, hc.dia, hc.Hora_Inicio, hc.Hora_Fin "
                ...
                + "JOIN matricula_Alumno ma ON hc.id = ma.id_horarioCurso "
                + "WHERE ma.id_alumno = (SELECT id FROM Alumno WHERE codigo_institucional = ?)";
